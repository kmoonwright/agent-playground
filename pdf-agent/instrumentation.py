...
def ensure_tracing(*, log_to_console: bool = False):
    ...
    Warns (does not crash) if Arize credentials are missing so local dry-runs
    ...
    if not space_id or not api_key:
        print(
            ...
            "Traces will not export to Arize AX. Pipeline will still run locally."
        )
    ...
    # SimpleSpanProcessor (batch=False) so traces land in AX during a live demo
...
def get_tracer():
    ...
    return trace.get_tracer("pdf-extraction-demo")
...
def using_document_context(doc_id: str, filename: str, extra: dict[str, Any] | None = None):
    """Attach document.id / filename to every child span (incl. auto-instrumented LLM)."""
    from openinference.instrumentation import using_attributes

    metadata = {"document_id": doc_id, "filename": filename}
    if extra:
        metadata.update(extra)
    tags = ["deduction-notice", doc_id]
    if extra and extra.get("is_trap"):
        tags.append("trap")
    return using_attributes(session_id=doc_id, metadata=metadata, tags=tags)
